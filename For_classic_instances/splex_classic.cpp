...
void showUsage(){
	fprintf(stderr, "splex -f <filename> -k <paramete k> -t <max seconds>  [-o optimum object] [-n threads]");
}
...
public:
	...
	void search_mode2 (int thread_id) {
        ...
        while(!end) {
			...
			while(1) {
                ...
                if (vec_M1.empty()) {
					/*if ((cur_splex->vnum < param_s) && (cur_splex->vnum < red_vnum)) {
						v = rand_r(&rand_seed) % red_vnum;
			 			while (is_in_c[v]) v = rand_r(&rand_seed) % red_vnum;
						add_cur_vertex(v);
						continue;
					}
					else */
			...
			reduce_num = is_reduced(red_min_deg); //best_size - param_s;   
	...
	void search_main(int thread_id){
		...
		// if (thread_id == 0) search_mode2(thread_id);
		// else 
