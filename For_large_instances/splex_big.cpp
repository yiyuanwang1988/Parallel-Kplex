...
void update_group_partition(int vnum, int id, vector<int>& vec_temp, int *freqs, long long *steps){
...
public:
	...
	void whether_update_group(int thread_id) {
		...
		if (freq_inc > freq_thres) {
			...
			if(!is_group_empty()) {
				update_group_partition(red_vnum, thread_id, vec_M1, freq, step_in_solution);
				time_update++;
			}
			freq_old = sumfreq;
		}
	...
	void tabu_based_search(int thread_id){
		...
		while (1){
			...
			while (!end){
				...
				//printf("satu_size %d core_perturb_flag %d non_improve_iter %d param_s * best_size2 %d\n", satu_size, core_perturb_flag, non_improve_iter, param_s * best_size2);
				/*           */
				if (core_perturb_flag == 1) {
					if (satu_size == 0) {
						core_perturb_flag = 2;
